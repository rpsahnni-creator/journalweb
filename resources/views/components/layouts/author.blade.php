...
@php
    $pageTitle = $title.' — Author — '.config('app.name');
    $journal = \App\Support\CurrentJournal::managed();
    $navItems = collect([
        ['label' => 'Dashboard', 'route' => 'author.dashboard', 'icon' => 'layout-dashboard', 'match' => 'author.dashboard'],
        ['label' => 'New manuscript', 'route' => 'author.manuscripts.create', 'icon' => 'plus-circle', 'match' => 'author.manuscripts.create'],
        ['label' => 'Submit manuscript', 'route' => 'submissions.create', 'icon' => 'upload', 'match' => 'submissions.create'],
        ['label' => 'My submissions', 'route' => 'submissions.index', 'icon' => 'inbox', 'match' => 'submissions.index'],
        ['label' => 'Author profile', 'route' => 'author.profile.edit', 'icon' => 'user', 'match' => 'author.profile.*'],
    ]);
@endphp
...
<head>
    ...
    <meta name="description" content="Author workspace for manuscript drafts, submissions, and profile details.">
...
<body class="min-h-screen bg-slate-100 font-sans text-slate-800 antialiased" x-data="{ sidebarOpen: false, userMenu: false }">
    ...
    >
        <div class="flex items-center justify-between gap-3 border-b border-white/10 px-5 py-4">
            <a href="{{ route('author.dashboard') }}" class="flex min-w-0 items-center gap-2.5 group">
                <div class="flex h-9 w-9 items-center justify-center rounded-lg bg-gradient-to-br from-brand-900 to-brand-800 text-accent-400 ring-1 ring-white/10 shadow-sm">
                    <x-icon name="pen-line" class="h-4.5 w-4.5" />
                </div>
                <div>
                    <span class="truncate font-serif text-base font-bold text-white block leading-tight">{{ $journal?->abbreviation ?: 'Author' }}</span>
                    <span class="text-[10px] uppercase font-semibold text-slate-400 tracking-wider">Author Console</span>
                </div>
            </a>
        ...
        <nav class="flex-1 overflow-y-auto px-3.5 py-4">
            <p class="px-2 pb-2 text-[11px] font-semibold uppercase tracking-wider text-slate-400">Manuscripts</p>
        ...
        <div class="border-t border-white/10 p-4 text-xs text-slate-400 bg-black/20">
            <p class="truncate font-medium text-slate-300">{{ auth()->user()?->name }}</p>
            <p class="mt-0.5 truncate">{{ $journal?->name ?? 'Author workspace' }}</p>
                ...
                <div class="flex min-w-0 items-center gap-3">
                    ...
                    <div class="min-w-0">
                        <div class="flex items-center gap-2">
                            <span class="inline-flex items-center gap-1 rounded px-1.5 py-0.5 text-[10px] font-bold uppercase tracking-wider bg-brand-50 text-brand-700 ring-1 ring-brand-200/50">Author Console</span>
                        </div>
                    ...
                    <div class="relative" @click.outside="userMenu = false">
                        ...
                        >
                            <div class="px-3.5 py-2">
                                ...
                                <p class="text-[10px] font-medium text-slate-500 uppercase tracking-wider">Author</p>
                            </div>
                            <div class="py-1">
                                <a href="{{ route('author.profile.edit') }}" class="flex items-center gap-2.5 px-3.5 py-2 text-xs font-medium text-slate-700 hover:bg-slate-50 hover:text-brand-900 transition-colors">
