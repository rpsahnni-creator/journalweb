<x-layouts.admin title="Add role">
    <p class="text-slate-600">Use a unique slug. Permissions control what assigned users can do outside of the administrator bypass.</p>

    <form method="POST" action="{{ route('admin.roles.store') }}" class="mt-6 max-w-2xl space-y-4 rounded-lg border border-slate-200 bg-white p-6">
        @csrf
            ...
            @forelse ($permissions as $permission)
                <label class="flex items-start gap-2 text-sm text-slate-700">
                    <input
                        ...
                        name="permission_ids[]"
                        value="{{ $permission->id }}"
                        @checked(in_array($permission->id, old('permission_ids', []), false))
        ...
        <div class="flex gap-3">
            <x-form.button :full="false">Create role</x-form.button>
